...
#include <math.h>
BLEScan* pBLEScan;  // Declare BLEScan globally
...
float thresholdDistance = 141.0;  // Threshold for ultrasonic sensor
String message = "";  
String Z = "N";  // Default detection status
String ultrasonicStatus = "0";  // Default ultrasonic sensor status
...
#endif
unsigned long lastObjectSendTime = 0;
...
Beacon beacons[] = {
  {"F4:B8:5E:A9:3A:76", -78, 0},  
  {"D0:5F:B8:30:3A:70", -72, 0},  
  {"D0:39:72:BC:5F:4C", -92, 0},  
  {"F8:36:9B:6C:4A:FA", -72, 0}   
};
...
String ultrasonicStatus = (distance <= thresholdDistance) ? "1" : "0";

if (ultrasonicStatus == "1") {
    digitalWrite(BUZZER_PIN, HIGH);
} else {
    digitalWrite(BUZZER_PIN, LOW);
}
for (int i = 0; i < 4; i++) beacons[i].detected = false;
pBLEScan->start(scanTime, false);

String dataToSend = " ";
for (int i = 0; i < 4; i++) {
  dataToSend += " " + String(beacons[i].rssi);
}

  String message = Z + ultrasonicStatus + dataToSend;
...
bool isConfirmed(String obj) {
    ...
    return count >= 1;  // Object must appear at least twice in 3 frames
}
